...
def _set_user_agent():
    ...
    #     return 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36
    #     (KHTML, like Gecko) Chrome/75.0.' + str(
...
class HTTPClient(object):
    ...
    def __init__(self, is_proxy, cdn_list=None):
        ...
        :param is_proxy:
        :param cdn_list: Must be a dict. Such as headers={'Content_Type':'text/html'}
        """
            ...
            try:
                ...
                if response.status_code == 200 or response.status_code == 302:
                    ...
                    if response.content:
                        ...
                        if urls["is_json"]:
                            return json.loads(
                                response.content.decode() if isinstance(response.content, bytes)
                                else response.content)
                        else:
                            return response.content.decode("utf8", "ignore") if \
                                   isinstance(response.content, bytes) else response.content
                    else:
                        print(f"url: {urls['req_url']}返回参数为空, 接口状态码:"
                        ...
                        if self.cdn_list:
                            ...
                            url_host = self.cdn_list.pop(random.randint(0, 4))
                        continue
                else:
                    sleep(urls["re_time"])
